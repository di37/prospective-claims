...
figures/07_make_report_figures__sweep_by_algorithm.png
```

The prefix is derived from the running script's filename rather than passed as an argument, so it cannot be forgotten or mistyped. `09_verify_invariants.py` fails on any artifact whose prefix matches no script in `scripts/`, which catches both an artifact written outside the harness and one left behind by a script that has since been renamed.
