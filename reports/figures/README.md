...
What is not allowed is a notebook cell that constructs a figure inline and saves it. That artifact exists only as a side effect of executing the notebook, no script regenerates it, and it goes stale silently when the results behind it change. `08_verify_invariants.py` checks for exactly that.
